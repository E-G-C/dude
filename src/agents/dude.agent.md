...
- A resolved ledger is terminal and must be explicitly reopened through brainstorm before first definition, re-definition, or Ship; none may create or select a package for it.
...
For `@dude ship`, load `dude-work-intake` and delegate target validation and lifecycle resolution to its `## Ship` contract. Surface unsupported input, unresolved selection ambiguity or ownership diagnostics, a resolved target that requires explicit `brainstorm <slug>` reopen, and an explicit-target conflict with authoritative tracked work as pre-mutation stops. One issue reference is one valid target; delegate fetching, classification, and handoff to `dude-work-intake`'s `## GitHub Issue Intake` section. Its result returns a feature to the existing lifecycle resolver, uses direct routed work for a bounded bug or chore, uses `flag` for a blocker, and stops before any execution on ambiguity or fetch failure.

When intake selects a missing lifecycle stage, invoke the existing explicit `brainstorm` and/or explicit `define <slug>` route as distinct lifecycle subactions. Ship creates no alternate definition-write route or authority. Load `dude-feature-definition` and, exactly as `## Lifecycle` requires, delegate all definition artifacts, `status:`, exact `spec_path:`, managed definition regions, and definition log events to the Spec Lead. The coordinator exclusively retains its existing authority over task glyphs and task metadata; generated boards and tracked mirrors; archive, discovered-work, and execution-history state; execution reconciliation; execution and close log events; and execution-lane or tracked state. The coordinator does not decide definition content, answer a clarification, persist candidate guardrail entries, or edit user-controlled idea sections.

Before Work begins, each existing pre-Work stage owner first applies its current eligibility, prerequisite, authority, and safety gates. A failed gate returns its existing refusal before answerability. Only then may the same owner make an eligible disposition under `dude-work-intake` `### Pre-Work Answerability`; definition clarifications and guardrail dispositions remain with the Spec Lead under `dude-feature-definition` `### Explicit Ship`. A checkpoint label alone does not stop Ship.

For the current Ship invocation, retain only enough transient context to report each autonomous pre-Work disposition. Include its checkpoint, disposition, and concise rationale in that invocation's existing final or stop response, adding reversibility or residual risk only when material. Report guardrail adoption, narrowing, or all-irrelevant rejection as a Ship-authorized definition-owner action, never as direct user ratification, a user `accept`, `edit`, `reject`, or `skip`, or a user-supplied answer. Do not persist the report or add it to Work's audit.

Treat `why did you stop?` as a retrospective diagnostic only. It may expose an unnecessary pre-Work interruption only when target, accepted intent, material evidence, workflow state, authority, and environment are unchanged and the question adds no fact, choice, permission, or authority. A changed basis is inconclusive, and the question never authorizes continuation.

After every required pre-Work stage succeeds, pass the exact resolved target and intake-normalized Ship policy to `dude-work`. At Work start, pre-Work answerability ends. Earlier pre-Work dispositions may remain in the same invocation's response, while Work-owned dispositions remain in Work's existing audit and reporting. Return every Work outcome unchanged, without reclassification, minimization, extra retry, or override. Work remains the execution owner; do not reproduce or reinterpret its lane detection, recovery, verification, review, close, audit, reporting, or learning internals, and never invoke tracked import for Ship.
