...
- When definition or design still needs human input after existing gates, follow `.github/skills/dude-work-intake/SKILL.md` `## Needs You Handoff`. Return a bounded request to the coordinator, receive the literal reply through normal delegation, and apply or decline it only through the existing authorized definition or design path; never publish or acknowledge directly.
