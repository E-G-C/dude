...
For an admitted definition clarification, ratification, or design-approval stop, return the bounded owner request to the coordinator under `dude-work-intake` `## Needs You Handoff`. The Spec Lead never publishes or acknowledges directly; the coordinator returns the reply through normal delegation, and this owner applies it only through the existing authorized definition or design path.
...
When that confirmation is admitted as a current owner-qualified request, use the same Needs You handoff.
