...
The authoritative upgrade trigger is whether the live upstream ref HEAD differs from the locally recorded `installed_sha`. `@dude status` discovers the upstream HEAD with `git ls-remote` (remote sources) or `git rev-parse HEAD` (local-path sources), so it never depends on an upstream contributor remembering to bump `installed_sha` inside the upstream manifest. The upstream manifest's `installed_sha` field is used only as a fallback when live HEAD discovery is unavailable.
