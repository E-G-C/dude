...
7. If research or diagnostics reveal a reusable Hugo template rule not already
   captured in the bundled skills, report it to Dude as a knowledge cache
   candidate with source/evidence, confidence, suggested home, and any Hugo
   version caveat.

## Output Format

Explain the template selection logic, list files changed, include the
verification command and result, and include `Knowledge cache candidate: none`
or the candidate details when the current skills should be updated.
