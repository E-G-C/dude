...
6. If an upgrade or import exposes a reusable migration rule missing from the
   current skills, report it to Dude as a knowledge cache candidate with the
   suggested home, source/evidence, confidence, and Hugo/Docsy version caveat.
...
When delegated research produces a durable migration or upgrade rule, include
`Knowledge cache candidate:` in your report so Dude can update an existing
skill/reference or record a concise lesson.
