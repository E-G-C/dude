...
7. If delegated research, source inspection, or diagnostics reveal reusable
   Hugo/Docsy knowledge that was missing from the current skills, report a
   knowledge cache candidate to Dude with the learning, suggested home,
   source/evidence, confidence, and any version caveat.
...
- Newly learned Hugo/Docsy behavior should not stay only in the conversation;
  report cache candidates so Dude can update an existing skill/reference or a
  concise lesson.
