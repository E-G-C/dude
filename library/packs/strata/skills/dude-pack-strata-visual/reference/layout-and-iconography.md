...
Use an 8-pt base scale. Tokens are exposed as `--strata-space-*`.
...
| `--strata-space-5` | 24 | between sections in a card |
| `--strata-space-6` | 32 | between cards |
...
| Token | Px | Use |
|---|---|---|
| `--strata-radius-sm` | 2 | inputs, small badges |
| `--strata-radius-md` | 4 | buttons, cards |
| `--strata-radius-lg` | 8 | dialogs, large cards |
| `--strata-radius-pill` | 9999 | tags, chips, status pills |
...
```css
--strata-elev-1: 0 1px 2px rgba(0,0,0,0.08);
--strata-elev-2: 0 2px 6px rgba(0,0,0,0.10);
--strata-elev-3: 0 4px 12px rgba(0,0,0,0.12);
```
...
|---|---|---|---|
| Primary | `--strata-blue` | white | none |
| Secondary | transparent | `--strata-text` | 1 px `--strata-border` |
| Subtle | `--strata-bg-subtle` | `--strata-text` | none |
| Destructive | `--strata-red` | white | none |

- Height: 32 px (compact), 40 px (default), 48 px (touch).
- Padding: 16 px horizontal at default height.
- Radius: `--strata-radius-md` (4 px).
...
- Pair icon + label with `--strata-space-2` (8 px).
...
- Focus rings must be visible — use a 2 px `--strata-blue` outline with 2 px offset.
