...
- Use only [Docsy-recognized keys](../skills/dude-pack-docsy-theme/SKILL.md#3-content-structure). Don't invent keys hoping they'll be picked up.
...
## Links
- **Never** use `ref` / `relref` to point at `_index.md` or `index.md` — they don't resolve. Use site-rooted relative links, for example `/your-section/`.
...
- Only use shortcodes documented in the [skill reference](../skills/dude-pack-docsy-theme/SKILL.md#5-shortcodes--complete-set). Don't fabricate parameters.
...
- Run `npm run _check:format` before committing; wrap Hugo directives Prettier mangles in `<!-- prettier-ignore-start -->` / `<!-- prettier-ignore-end -->`.
