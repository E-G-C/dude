...
2. Read the narrowest relevant bundled reference file. For functions/methods use `dude-local-hugo-functions-and-methods`; for asset pipelines use `dude-local-hugo-asset-pipeline`; for shortcodes, content features, install, or hosting use the matching `dude-local-hugo-docs-reference` reference file.
