...
- **Import the tokens** before adding branded styles:
  - CSS: `@import "../skills/dude-local-ms-visual-brand/tokens/ms-brand.css";`
    from `.github/instructions/`, or use the correct relative path from the
    target file.
  - SCSS:
    `@use "../skills/dude-local-ms-visual-brand/tokens/ms-brand.scss" as ms;`
    from `.github/instructions/`, or use the correct relative path from the
    target file.
  - Tailwind: extend the config with
    `.github/skills/dude-local-ms-visual-brand/tokens/tailwind.preset.js`.
...
- For internal work, `.ms-logo-mark` from
  `.github/skills/dude-local-ms-visual-brand/tokens/ms-brand.css` is the
