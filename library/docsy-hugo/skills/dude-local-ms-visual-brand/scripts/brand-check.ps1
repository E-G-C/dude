...
# `@dude-local-ms-brand-stylist` agent for visual review.
#
# Usage:  pwsh .github/skills/dude-local-ms-visual-brand/scripts/brand-check.ps1
...
$tokenImport = 'dude-local-ms-visual-brand/tokens/ms-brand.scss'
...
foreach ($glob in $searchGlobs) {
    ...
    $matches = Get-ChildItem -Path $glob -Recurse -File -ErrorAction SilentlyContinue |
        ...
        Where-Object { $_.Path -notmatch 'dude-local-ms-visual-brand[\/]+tokens[\/]+' }
