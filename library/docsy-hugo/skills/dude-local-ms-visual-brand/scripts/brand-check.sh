...
#
# Usage:  bash .github/skills/dude-local-ms-visual-brand/scripts/brand-check.sh
...
token_import='dude-local-ms-visual-brand/tokens/ms-brand.scss'
...
for glob in "${search_globs[@]}"; do
  ...
  if matches="$(grep -RInEi "$brand_hex" "$glob" 2>/dev/null \
      | grep -viE 'dude-local-ms-visual-brand/tokens/' || true)"; then
    if [ -n "$matches" ]; then
      hits+="$matches"$'\n'
    fi
  fi
