...
7. **Roster orphans**
   - Collect every `@<role>` reference under `.github/` (excluding fenced code blocks, inline code spans, including indented fences, and the `dude` and `dude-lint` allowlist).
