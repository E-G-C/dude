...
- Transaction material cannot be created exclusively under a never-reused identity, remain contained within the workspace, remain disjoint from the source, every output, every other transaction, and between its staging and backup areas, exclude links, retain expected path types, preserve its exact transaction identity, or satisfy the platform privacy policy; staging, backup, destination mutation, rollback, or recovery cleanup then fails.
...
- **FR-015**: Optional review input MUST contain only its schema identity, exact `analysis_sha256`, sorted unique `reviewed_batch_ids`, and advisory findings naming one reviewed batch ID and a valid path in that exact batch with category, `info` or `warn` severity, evidence, and explanation. A reviewed batch ID means its complete generated batch was reviewed. The complete optional review JSON MUST be at most 1,048,576 UTF-8 bytes: oversized raw bytes MUST be rejected before JSON parsing, and an oversized structured review's canonical JSON MUST be rejected before semantic acceptance. This fixed versioned v1 renderer/validator fact MUST accept no caller override, MUST NOT be serialized, and MUST remain aggregate-only, with no per-finding count, evidence, explanation, path, or generic scalar limits.
...
- **FR-023**: The complete canonical reviewed plan, including `plan_sha256`, MUST be at most 16,777,216 UTF-8 bytes and MUST be rejected before emission or structured acceptance when oversized; apply MUST reject oversized raw plan bytes before parsing or acceptance. This fixed versioned v1 renderer/validator fact MUST accept no caller override, MUST NOT be serialized, and MUST remain aggregate-only, with no per-finding count, evidence, explanation, path, or generic scalar limits. Apply MUST validate the plan digest and accept exactly once either `confirm-import` for Clean or the literal `confirm-warned-import:` plus the final `plan_sha256` for Warned, then recompute every deterministic source, output, and destination fact and refuse any mismatch before the first destination mutation.
...
- **FR-025**: Apply MUST install only exact reviewed bytes and paths and provide verified rollback or an explicit recovery-failure result naming every uncertain path without claiming success. All transaction material MUST be created exclusively under a never-reused transaction identity within the workspace, remain disjoint from the source, every output, every other transaction, and between its staging and backup areas, contain no links, retain expected path types, and be revalidated against the exact transaction identity before use. On POSIX, every transaction directory and regular file MUST be owned by the current UID, with directories mode `0700` and regular files mode `0600`. On Windows, regular transaction directories and files MUST use the selected transaction parent's inherited ACLs; apply MUST NOT manage or rewrite ACLs and MUST NOT claim current-user-only access. Users requiring stronger privacy MUST secure the selected parent externally before apply.
...
- **SC-002**: Fixtures at and beyond every fixed depth, entry, file-count, per-file, aggregate-byte, response, timeout, optional-review JSON byte, and reviewed-plan JSON byte boundary produce the documented result with zero destination writes.
...
- **SC-005**: Optional-review raw bytes and structured-review canonical JSON at exactly 1,048,576 UTF-8 bytes are accepted when otherwise valid; 1,048,577 raw bytes are rejected before JSON parsing, and 1,048,577 bytes of canonical JSON are rejected before semantic acceptance. Every valid reviewed batch ID covers exactly its generated complete batch; omitted IDs remain visibly derivable as uncovered, and any stale, malformed, unknown, duplicate, over-budget, opaque, or uncovered review condition prevents Clean.
...
- **SC-008**: A complete canonical reviewed plan including `plan_sha256` at exactly 16,777,216 UTF-8 bytes is accepted when otherwise valid, while 16,777,217 bytes are rejected before emission or structured acceptance; raw apply-plan bytes at exactly 16,777,216 are accepted when otherwise valid, while 16,777,217 are rejected before parsing or acceptance. Every tested source, output, and transaction containment, alias, file-identity, source-drift, destination-drift, and confirmation mismatch refuses before the first destination mutation.
- **SC-009**: On every platform, transaction-privacy fixtures verify exclusive never-reused creation within the workspace, source/output/other-transaction and staging/backup disjointness, no links, expected path types, and exact transaction identity. POSIX fixtures also verify current-UID ownership, `0700` directories, and `0600` regular files; Windows fixtures verify selected-parent inherited ACL behavior without ACL management or a current-user-only claim. Failure injection at each privacy check, staged mutation, and rollback point yields exact planned installation, verified restoration, or explicit recovery failure with every uncertain path and no false success.
