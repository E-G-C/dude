...
7. **Given** an outstanding Assessment challenge bound to a fresh Inspection, **When** the model-produced response contains a trailing-slash material target, another malformed or missing material field, a stale otherwise-valid evidence hash, or a valid Assessment, **Then** the producing host validates before returning it and the runner independently validates it, classifies malformed input as `challenge-response-invalid` with only bounded safe detail, reserves `challenge-response-stale` for the otherwise-valid stale evidence hash, and accepts the valid Assessment while both rejection classes preserve accepted state, counters, pending authority, and task and lane state with no authorization, implementation, retry, or cleanup attempt.
...
- An Assessment is structurally recognizable but carries an otherwise-valid stale evidence hash, versus carrying a malformed hash, a missing or malformed material field, or a material target with an empty path segment; only the first case is stale, while every other malformed case is invalid and may expose only an allowlisted field locator and failure class.
...
- **FR-031:** The producing host integration MUST validate every model-produced Assessment against the exact outstanding target and Inspection before returning its challenge response. The runner MUST independently apply the same authoritative Assessment validation to every received Assessment because the response remains untrusted.
- **FR-032:** `challenge-response-stale` MUST be reserved for a structurally recognizable Assessment whose only invalid binding is an otherwise-valid 64-hex evidence hash unequal to the current Inspection. Every other malformed Assessment field or envelope MUST be `challenge-response-invalid`. An invalid result MAY carry only bounded allowlisted detail identifying a known field location and failure class, such as `Assessment.materialInputs.targets[3]: empty-path-segment`, and MUST NOT include rejected target text, evidence bodies, hashes, arbitrary validator messages or values, credentials, or paths from the rejected field.
- **FR-033:** A stale or invalid Assessment challenge response MUST retain the existing terminal orphan hard stop with cleanup not attempted; preserve accepted state bytes, revisions, counters, pending authority, and task and lane state; perform no authorization or implementation; and add no automatic retry, same-challenge correction, validator weakening, path normalization, retry budget, checkpoint state, cleanup behavior, takeover, or route. Existing foreign, replayed, and out-of-order response protections MUST remain unchanged.
...
- **SC-017:** In the Assessment challenge-response matrix, every malformed Assessment, including any malformed or missing material field and a trailing-slash material target, produces `challenge-response-invalid` with the exact fixed safe detail `Assessment: invalid-contract`; an otherwise-valid Assessment whose sole defect is a noncurrent 64-hex evidence hash produces `challenge-response-stale`; and a valid Assessment is accepted. No rejection exposes target text, evidence bodies, hashes, arbitrary validator messages or values, credentials, or rejected paths.
- **SC-018:** In 100% of stale and invalid Assessment rejection fixtures, producing-host prevalidation and independent runner validation are both exercised; accepted state bytes, revisions, counters, pending authority, and task and lane state remain unchanged; cleanup is `not-attempted`; authorization and implementation calls are zero; and no retry or same-challenge correction occurs. Existing foreign, replayed, and out-of-order fixtures retain their prior outcomes.
