...
//
// It drives the real design host with native pointer input over CDP, then calls
// the engine's own public `update` boundary -- the same call
// `ReviewWorkspace` makes with `unavailable: !current` -- without moving the
// pointer, so the observed cursor is the stationary-pointer case in the finding.
...
const CASES = [
  { name: 'drawing tool armed over a selected handle', tool: 'box', at: 'handle',
    hoverAction: 'nwse-resize', toolCursor: 'crosshair' },
  { name: 'Select over a selected border', tool: 'select', at: 'border',
    hoverAction: 'move', toolCursor: 'default' },
];
...
async function runCase(page, plan) {
  ...
  try {
    ...
    assert.equal(sameOverlay, true, 'the hovered overlay node survived the update');
    assert.equal(after.stale, false, 'the update reported unavailable, not a stale source');
    assert.equal(after.editable, false, 'the update blocked editing');
    ...
    return {
      tool: plan.tool, point: plan.at, loadedEngine, hovered, blocked, afterPress, sameOverlay,
