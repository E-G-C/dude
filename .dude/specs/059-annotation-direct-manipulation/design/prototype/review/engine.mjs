...
export function mountReview(container, { review, requestHandle, revision, theme, onChange = () => {}, onMessage = () => {} }) {
  ...
  function reportError(error) {
    ...
    if (['already_consumed', 'provider_unavailable', 'unknown_request', 'review_historical'].includes(error.code)) {
      ...
      // 059 exploration. The same invariant as the update path: cancelDrag is a
      // no-op under a resting pointer and message only emits, so the transient
      // action cursor is refreshed here too. A refusal that already happened
      // must not keep promising the move or resize the next press would refuse.
      refreshAction();
    }
  ...
  function update(next) {
    ...
    if (next.unavailable) {
      ...
      editable = false; status = 'unavailable'; clearTimeout(saveTimer); cancelDrag();
      // 059 exploration. cancelDrag is a no-op under a resting pointer, so the
      // transient action cursor is refreshed here too: a state that now refuses
      // editing must not keep promising the move or resize it would refuse.
      refreshAction();
    }
