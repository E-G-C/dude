...
/**
 * 059 exploration. The drawing-mode grab predicate for the one selected shape:
 * its own painted edge, ring, or stroke, and nothing else. Snagit's selected
 * object answers on its border while the tool stays armed, so the reviewer can
 * adjust the mark and keep drawing. Deliberately narrower than `hitTest`: no
 * badge (a number is not a border), no whole box/highlight face (the interior
 * stays drawing space), and no enclosing rectangle for a circle or segment.
 *
...
/**
 * 059 exploration. The native cursor for one existing handle, so the pointer
 * describes the resize it will actually perform. Corners follow the platform's
 * diagonal pair; a segment endpoint follows its own direction, the way a
 * drawing tool points a line's end handle along the line.
 */
