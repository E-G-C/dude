/**
 * T008 rev-5.1 · unapproved, memory-only design artifact.
 *
...
import {
  ...
  ToolbarRadioGroup, ToolbarRadioButton, Listbox, Option,
...
function CommandBar({ onNeeds, onNew, onRefresh, onDetails, readOnly = false }) {
  const s = useStyles();
  return <Toolbar aria-label="Workspace commands" className={s.commands} data-review-key="commands">
    <ToolbarGroup>
      <ToolbarButton icon={<CommentRegular />} onClick={readOnly ? undefined : onNeeds} data-review-key="command-needs">Needs you</ToolbarButton>
  ...
  return <div className={s.contextBar} data-review-key="context-picker">
    <Field label="Context" className={s.contextField}>
      <Combobox className={s.control} listbox={{ className: s.contextOptions }}
        ...
        {options.map(c => <Option key={c.ideaPath} value={c.ideaPath} text={c.title} className={s.option}>
    ...
    <div className={mergeClasses(s.page, s.app)}>
      ...
      <div className={mergeClasses(s.viewport, settings.width !== 'fluid' && s[`width${settings.width}`])}>
        <section className={mergeClasses(s.product, reviewOpen && s.hidden)} aria-label="Needs you workspace">
          <CommandBar onNeeds={openNeeds} onNew={newIdea} onRefresh={() => {
