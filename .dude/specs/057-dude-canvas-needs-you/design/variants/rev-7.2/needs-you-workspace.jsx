/**
 * T008 rev-7.2 · unapproved, memory-only design artifact.
 *
...
function ContextBar({ projection, selected, onCommit, onBrowse, readOnly = false }) {
  ...
  return <div className={s.contextBar} data-review-key="context-picker">
    <Field label={{ children: 'Context', className: s.live }} className={s.contextField}>
...
function Overview({ snapshotKey, projection, onOpen, onBrowse, onNew, readOnly = false }) {
  const s = useStyles();
  const sectionId = useId();
  const browseFocusTarget = useRestoreFocusTarget();
  ...
  return <div className={s.overview} data-review-key="overview">
    <header className={s.between}>
      ...
      <Button {...browseFocusTarget} onClick={readOnly ? undefined : onBrowse} data-review-key="overview-browse">Browse all work</Button>
    </header>
    {error ? <Notice intent="warning" title="Overview unavailable">{error}</Notice>
      ...
      <div className={s.overviewColumns}>
        {snapshot.state === 'current' && <section className={s.overviewCurrent} aria-labelledby={`${sectionId}-current`}>
          ...
          {currentRows.length ? <>
            ...
            {currentRows.length > 3 && <Text className={s.eyebrow}>More current work is listed in Browse all work.</Text>}
        ...
        <section className={s.overviewPlanned} aria-labelledby={`${sectionId}-planned`}>
          ...
          {plannedGroups.length ? plannedGroups.map(([key, , label]) => <div key={key} className={s.tight}>
            ...
            {snapshot.planned[key].length > 3 && <Text className={s.eyebrow}>More planned work is listed in Browse all work.</Text>}
      ...
      {snapshot.state === 'current' && <section className={s.overviewHistory} aria-labelledby={`${sectionId}-history`}>
        ...
        <Text className={s.eyebrow}>Browse all work lists every recorded idea and feature.</Text>
      </section>}
    </>}
  </div>;
}
...
function ContextDetail({ context, projection, onReview, readOnly = false, reviewCondition = 'editing' }) {
  ...
  if (!context) return <div className={s.measure}>
    ...
    <p className={s.lead}>Choose an idea above or use Browse to inspect all work. Needs you shows requests across the workspace.</p>
  </div>;
    ...
    <div className={mergeClasses(s.page, s.app)}>
      ...
      <div className={mergeClasses(s.viewport, settings.width !== 'fluid' && s[`width${settings.width}`])}>
        <section className={mergeClasses(s.product, reviewOpen && s.hidden)} aria-label="Needs you workspace">
          ...
          {view === 'context' && <ContextBar projection={projection} selected={contextId}
            ...
            <div className={s.stack}>
              ...
              {view === 'new' ? <NewIdea value={value} onChange={updateDraft} onCancel={openNeeds} />
                : view === 'overview' ? <Overview snapshotKey={settings.snapshot} projection={projection}
                  onOpen={openOverviewContext} onBrowse={() => setBrowseOpen(true)} onNew={newIdea} />
