/**
 * T008 rev-4 · unapproved, memory-only design artifact.
 *
...
import {
  ...
  DismissRegular, SearchRegular, MoreHorizontalRegular, DeleteRegular,
...
const REVISION = 'rev-4';
const DATA = globalThis.DUDE_MOCK_DATA;
const EMPTY = { label: 'Snapshot not loaded', projection: { contexts: [], coverage: {}, sources: [] } };
const SNAPSHOTS = DATA?.snapshots || { workspace: EMPTY };
const CURRENT = SNAPSHOTS.workspace?.projection || EMPTY.projection;
...
const RECORDED_BOUNDARIES = {
  ...
  '.dude/ideas/056-ship-orphan-cleanup.md': { title: 'Recorded as deferred', text: 'The cleanup idea remains discoverable without an urgent request.', source: '.dude/memory/context.md · Future Canvas rediscovery', terminal: false },
};
...
const EXAMPLES = [
  ...
  { id: 'preview', title: 'Is the next action clear in this layout?', type: 'Design review', subject: 'Dude Canvas Needs You',
    why: 'Review the actual rev-4 proposal. Feedback asks for a design revision; approval is a separate decision.',
...
function Discovery({ projection, tab, onTab, query, onQuery, filter, onFilter,
  selected, onSelect, examples, drafts, readOnly = false, showDetail, noRequestsExample = false }) {
  const s = useStyles();
  ...
  const visible = rows.filter(row => {
    ...
    return match && (tab === 'needs' || filter === 'all' || (filter === 'draft' ? row.status === 'draft' : row.kind === 'feature'));
  });
  useEffect(() => {
  ...
  return <aside aria-label="Discovery" className={mergeClasses(s.navigation, showDetail && s.mobileHideNav)}>
    <div className={s.navHead}>
      ...
      {tab === 'ideas' && <SelectField label="Show" value={filter} onChange={v => onFilter?.(v)}
        options={[['all', 'All ideas & features'], ['draft', 'Draft ideas'], ['feature', 'Defined features']]} />}
...
function ContextDetail({ context, projection, onReview, onDetails, readOnly = false, reviewCondition = 'editing' }) {
  ...
  const terminal = ['closed', 'complete', 'completed'].includes(context.status) || boundary?.terminal;
  return <div className={s.measure}>
    <header className={s.detailHeader}>
      <Text className={s.eyebrow} data-review-key="context-kind">{context.status === 'draft' ? 'Draft idea' : 'Defined feature'}</Text>
    ...
    </div> : terminal && <Notice intent="success" title="Closed in the source">This entry is retained for discovery. It does not reopen work.</Notice>}
    ...
    {context.status === 'draft' && <Disclosure title="Before visual review">
...
function WorkspaceDocument() {
  const s = useStyles();
  return <div className={mergeClasses(s.subject, s.product)} aria-label="Read-only rev-4 workspace proposal">
...
function RequestForm({ request, value, onChange, responseState, permission, reviewCondition, onReview, onApprove, onPrepared, onDefer, onSave }) {
  const s = useStyles();
  ...
  return <div className={s.measure}>
    ...
    {!editable ? <div className={s.stack}>
      ...
      {request.id === 'preview' && <>
        ...
        {['missing', 'owner', 'draft'].includes(reviewCondition) ? <Notice intent="warning" title="Review prerequisite">
      ...
      <div className={s.actions}>
        ...
        <Menu>
          <MenuTrigger disableButtonEnhancement><Button icon={<MoreHorizontalRegular />}>More</Button></MenuTrigger>
          <MenuPopover><MenuList>
            <MenuItem onClick={onDefer}>Defer…</MenuItem>
            {request.id === 'onboarding' && !!value.intent?.trim() && <MenuItem onClick={onSave}>Save as idea…</MenuItem>}
...
function NewIdea({ value, onChange, onPrepared, onDefer, onSave }) {
  const s = useStyles();
  const [attempted, setAttempted] = useState(false);
  return <form className={s.measure} onSubmit={e => {
    ...
    <div className={s.actions}>
      ...
      <Menu><MenuTrigger disableButtonEnhancement><Button icon={<MoreHorizontalRegular />}>More</Button></MenuTrigger>
        <MenuPopover><MenuList><MenuItem onClick={onDefer}>Defer…</MenuItem>
          {!!value.intent?.trim() && <MenuItem onClick={onSave}>Save as idea…</MenuItem>}
        </MenuList></MenuPopover>
      </Menu>
    </div>
...
function IntentDialog({ kind, text, onClose, onPrepared, sourceBacked }) {
  const s = useStyles();
      ...
      <DialogActions>
        ...
        <Button appearance="primary" onClick={() => {
          ...
          onPrepared({ title: kind === 'save' ? 'Idea capture prepared' : 'Deferral prepared',
            ...
            payload: kind === 'save' ? { action: 'save_as_idea', intent } : { action: 'defer', sourceBacked } });
          onClose();
        }}>{kind === 'save' ? 'Prepare capture' : 'Prepare deferral'}</Button>
      </DialogActions>
    </DialogBody></DialogSurface>
  </Dialog>;
}
...
function App() {
  ...
  const snapshot = SNAPSHOTS[settings.snapshot] || EMPTY, projection = snapshot.projection;
  ...
  return <FluentProvider theme={THEMES[settings.theme]} className={s.provider}>
    <div className={s.app}>
        ...
        <section className={mergeClasses(s.product, reviewOpen && s.hidden)} aria-label="Needs you workspace">
          ...
          <div className={s.panes}>
            <Discovery projection={projection} tab={tab} onTab={switchTab}
              ...
              examples={settings.mode === 'examples'} drafts={drafts} showDetail={showDetail} noRequestsExample={settings.coverage === 'current'} />
            <main ref={detailRef} tabIndex={-1} className={mergeClasses(s.detail, !showDetail && s.mobileHideDetail)}>
              <div className={s.stack}>
                ...
                {isNew ? <NewIdea value={value} onChange={updateDraft} onPrepared={setPrepared} onDefer={() => setIntentDialog('defer')} onSave={() => setIntentDialog('save')} />
                  : request ? <RequestForm key={`${request.id}-${settings.permission}`} request={request} value={value} onChange={updateDraft}
                    ...
                    onDefer={() => setIntentDialog('defer')} onSave={() => setIntentDialog('save')} />
      ...
      {intentDialog && <IntentDialog kind={intentDialog} text={value.intent} sourceBacked={!isNew && request?.id !== 'onboarding'}
        onClose={() => setIntentDialog(null)} onPrepared={setPrepared} />}
