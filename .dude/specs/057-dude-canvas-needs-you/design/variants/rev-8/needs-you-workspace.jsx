...
const WORK_SCOPES = [['open', 'Open work'], ['all', 'All work'], ['history', 'History']];
...
function WorkList({ rows, projection, workStatus, history, onOpen, readOnly = false, finder, onFinder, scrollRef }) {
  ...
  const historyCaption = active === 'history' && history
    ? `${visible.length} of ${scopedWorkRows(rows, 'history').length} finished records · ${history.completedPackages} completed features and ${history.resolvedIdeas} resolved ideas`
  ...
  return <section ref={sizeRef} className={s.stack} aria-labelledby={`${listId}-heading`}>
    ...
    {workStatus && <Notice intent="warning" title="Recorded work status is unavailable">
      {workStatus} Every recorded idea and feature is still listed here, without task progress or history counts.
    </Notice>}
    ...
    <div className={s.tight}>
      ...
      {history && active !== 'history' && <Text className={s.eyebrow} data-review-key="overview-history-count">
        History holds {history.completedPackages} completed features and {history.resolvedIdeas} resolved ideas.
        Resolved ideas are terminal history, not delivered features.
      </Text>}
