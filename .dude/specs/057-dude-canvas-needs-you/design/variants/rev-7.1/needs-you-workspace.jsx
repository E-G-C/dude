/**
 * T008 rev-7.1 · unapproved, memory-only design artifact.
 *
 ...
 * - Overview: DUDE_OVERVIEW_DATA, bound to that base snapshot and selected root.
 ...
 * - Review document: the same Overview/ContextBar/CommandBar components as
...
const OVERVIEW_CURRENT_GROUPS = [
  ['blocked', 'Marked blocked'], ['active', 'Marked in progress'], ['next', 'Planned next'],
];
const OVERVIEW_PLANNED_GROUPS = [
  ['awaitingDefinition', 'awaiting-definition', 'Draft ideas awaiting definition'],
  ['definedAwaitingWork', 'defined-awaiting-work', 'Defined, awaiting work'],
  ['prioritizedLater', 'prioritized-later', 'Planned for later'],
];

// Validate only this read-only overview. Failure must not disable the other views.
function overviewLoadError(data, snapshotKey, projection) {
  const malformed = 'The overview snapshot could not be read. Context, Browse, and New idea are still available.';
  const mismatch = 'The overview does not match this workspace snapshot. Use Context or Browse for the recorded ideas.';
  const count = value => Number.isSafeInteger(value) && value >= 0;
  const nullableCount = value => value === null || count(value);
  const date = value => typeof value === 'string' && Number.isFinite(Date.parse(value));
  if (!data) return 'The overview snapshot was not loaded. Context, Browse, and New idea are still available.';
  ...
  const { state, basis, summary, current, planned } = snapshot;
  const workRead = state === 'current';
  const historyKeys = ['completedPackages', 'resolvedIdeas'];
  if (!['current', 'empty', 'inventory-only'].includes(state) || !date(snapshot.readAt)
    || !summary || !historyKeys.every(key => nullableCount(summary[key]))
    || !planned || !Array.isArray(planned.awaitingDefinition)) return malformed;
  ...
  if (workRead) {
    if (!current || !OVERVIEW_CURRENT_GROUPS.every(([key]) => Array.isArray(current[key]))
      || !OVERVIEW_PLANNED_GROUPS.every(([key]) => Array.isArray(planned[key]))) return malformed;
  } else if (current !== null || planned.definedAwaitingWork !== null || planned.prioritizedLater !== null
    || !historyKeys.every(key => summary[key] === null)) return malformed;
  const seen = new Set();
  const groups = [
    ...OVERVIEW_CURRENT_GROUPS.map(([key]) => [current?.[key], key]),
    ...OVERVIEW_PLANNED_GROUPS.map(([key, group]) => [planned[key], group]),
  ];
  for (const [rows, group] of groups) {
    if (rows == null) continue;
    if (!Array.isArray(rows)) return malformed;
    for (const row of rows) {
      if (!row || typeof row.ideaPath !== 'string' || typeof row.title !== 'string'
        || typeof row.defined !== 'boolean' || row.group !== group) return malformed;
      const matches = projection.contexts.filter(context => context.ideaPath === row.ideaPath);
      if (matches.length !== 1 || seen.has(row.ideaPath)) return mismatch;
      const context = matches[0];
      if (context.title !== row.title || context.status !== row.status
        || row.defined !== (context.kind === 'feature' && !!context.specPath)) return mismatch;
      seen.add(row.ideaPath);
      if (row.taskCounts !== null) {
        const tasks = row.taskCounts;
        if (!workRead || !tasks || !count(tasks.done) || !count(tasks.total) || tasks.done > tasks.total) return malformed;
      }
    }
  }
  return null;
...
const useStyles = makeStyles({
  ...
  overviewColumns: { display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', gap: tokens.spacingHorizontalXL },
  overviewCurrent: { flex: '2 1 360px', minWidth: 0, display: 'flex', flexDirection: 'column', gap: tokens.spacingVerticalM },
  overviewPlanned: { flex: '1 1 280px', minWidth: 0, display: 'flex', flexDirection: 'column', gap: tokens.spacingVerticalM },
  overviewCard: { minWidth: 0 },
  overviewTitle: { overflowWrap: 'anywhere' },
  overviewList: { listStyleType: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column' },
  overviewRow: {
  ...
  overviewRowTitle: { flex: 1, minWidth: 0, overflowWrap: 'anywhere' },
  overviewHistory: {
    borderTop: `${tokens.strokeWidthThin} solid ${tokens.colorNeutralStroke2}`,
    paddingTop: tokens.spacingVerticalL, display: 'flex', flexDirection: 'column', gap: tokens.spacingVerticalM,
  },
...
function CommandBar({ view, viewId, hasContext, onOverview, onContext, onNeeds, onNew, onRefresh, readOnly = false }) {
  const s = useStyles();
...
function NeedsYou({ examples, noRequestsExample, query, onQuery, selected, onSelect, drafts, responseState }) {
  ...
  return <div className={s.measure}>
    ...
    {rows.length ? <Listbox className={s.requestList} aria-label="Example requests"
      ...
      <p className={s.prose}>{noRequestsExample ? 'The simulated scope is fully read. Open Overview to browse recorded ideas and deferred work.'
        : examples ? 'Try a different search.'
          : 'The snapshot does not contain an admitted request feed. It cannot establish that nothing needs you. Open Overview to browse recorded work.'}</p>
    </section>}
  </div>;
}

function OverviewWorkItem({ row, label, onOpen, readOnly }) {
  const s = useStyles();
  const titleId = useId();
  const progressId = useId();
  const tasks = row.taskCounts;
  return <Card className={s.overviewCard} aria-labelledby={titleId} data-review-key={`overview-${row.group}-${row.ideaPath}`}>
    <CardHeader header={<h3 id={titleId} className={mergeClasses(s.title, s.overviewTitle)}>{row.title}</h3>}
      description={<Text>{label}</Text>}
      action={<Button appearance="primary" aria-label={`Open ${row.title}`}
        onClick={readOnly ? undefined : () => onOpen(row.ideaPath)}>Open</Button>} />
    <div className={s.tight}>
      <Text id={progressId}>{tasks === null ? 'Task progress unavailable in this snapshot.'
...
function Overview({ snapshotKey, projection, onOpen, onNew, readOnly = false }) {
  const s = useStyles();
  const sectionId = useId();
  const error = overviewLoadError(OVERVIEW_DATA, snapshotKey, projection);
  const snapshot = error ? null : OVERVIEW_DATA.snapshots[snapshotKey];
  const currentRows = snapshot?.current ? OVERVIEW_CURRENT_GROUPS.flatMap(([key, label]) =>
    snapshot.current[key].map(row => ({ row, label }))) : [];
  const plannedGroups = snapshot ? OVERVIEW_PLANNED_GROUPS.filter(([key]) => snapshot.planned[key]?.length) : [];
  return <div className={s.overview} data-review-key="overview">
    <header className={s.detailHeader}>
      <h1 className={s.title}>Overview</h1>
      {snapshot && <Text className={s.eyebrow}>
        {SNAPSHOTS[snapshotKey].fixture ? 'Example snapshot' : 'Recorded snapshot'} · <time dateTime={snapshot.readAt}>
          {new Date(snapshot.readAt).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}
        </time> · Not live activity
      </Text>}
    </header>
    {error ? <Notice intent="warning" title="Overview unavailable">{error}</Notice>
      : snapshot.state === 'empty' ? <section className={s.stack} aria-labelledby={`${sectionId}-empty`}>
        <h2 id={`${sectionId}-empty`} className={s.subheading}>No recorded ideas or features yet</h2>
        <div className={s.actions}><Button onClick={readOnly ? undefined : onNew}>New idea</Button></div>
      </section> : <>
      <div className={s.overviewColumns}>
        {snapshot.state === 'current' && <section className={s.overviewCurrent} aria-labelledby={`${sectionId}-current`}>
          <h2 id={`${sectionId}-current`} className={s.subheading} data-review-key="overview-current-heading">Current</h2>
          {currentRows.length ? <>
            {currentRows.slice(0, 3).map(({ row, label }) =>
              <OverviewWorkItem key={row.ideaPath} row={row} label={label} onOpen={onOpen} readOnly={readOnly} />)}
            {currentRows.length > 3 && <Text className={s.eyebrow}>More current work is listed in Browse.</Text>}
          </> : <Text>No current work is recorded in this snapshot.</Text>}
        </section>}
        <section className={s.overviewPlanned} aria-labelledby={`${sectionId}-planned`}>
          <h2 id={`${sectionId}-planned`} className={s.subheading} data-review-key="overview-planned-heading">Planned</h2>
          {plannedGroups.length ? plannedGroups.map(([key, , label]) => <div key={key} className={s.tight}>
            <Text weight="semibold">{label} ({snapshot.planned[key].length})</Text>
            <ul className={s.overviewList} aria-label={label}>
              {snapshot.planned[key].slice(0, 3).map(row => <li key={row.ideaPath} className={s.overviewRow}
                data-review-key={`overview-${row.group}-${row.ideaPath}`}>
                <Text className={s.overviewRowTitle}>{row.title}</Text>
                <Button appearance="subtle" aria-label={`Open ${row.title}`}
                  onClick={readOnly ? undefined : () => onOpen(row.ideaPath)}>Open</Button>
              </li>)}
            </ul>
            {snapshot.planned[key].length > 3 && <Text className={s.eyebrow}>More planned work is listed in Browse.</Text>}
          </div>) : <Text>No draft ideas are recorded in this snapshot.</Text>}
          {snapshot.state === 'inventory-only' && <Text className={s.eyebrow}>Task progress and history are unavailable in this snapshot.</Text>}
          <div className={s.actions}><Button onClick={readOnly ? undefined : onNew}>New idea</Button></div>
        </section>
      </div>
      {snapshot.state === 'current' && <section className={s.overviewHistory} aria-labelledby={`${sectionId}-history`}>
        <h2 id={`${sectionId}-history`} className={s.subheading} data-review-key="overview-history-heading">History</h2>
        <div className={s.row}>
          <Text>{snapshot.summary.completedPackages === null ? 'Completed features: unavailable'
            : `${snapshot.summary.completedPackages} completed features`}</Text>
          <Text>{snapshot.summary.resolvedIdeas === null ? 'Resolved ideas: unavailable'
            : `${snapshot.summary.resolvedIdeas} resolved ideas`}</Text>
        </div>
        <Text className={s.eyebrow}>Browse includes every recorded idea and feature.</Text>
      </section>}
    </>}
  </div>;
}

function ContextDetail({ context, projection, onReview, readOnly = false, reviewCondition = 'editing' }) {
  const s = useStyles();
  ...
  return <div className={s.measure}>
    ...
    <section className={s.stack}>
      ...
      {context.intent?.truncated && <Text className={s.eyebrow}>Only an excerpt is shown. Browse lists the original idea file.</Text>}
    </section>
...
function WorkspaceDocument() {
  const s = useStyles();
  const viewId = useId();
  const selected = CURRENT.selected?.explicit ? CURRENT.selected.ideaPath : null;
  return <div className={mergeClasses(s.subject, s.product, s.overviewDocument)} aria-label={`Read-only ${REVISION} workspace proposal`}>
    ...
    <CommandBar view="overview" viewId={viewId} hasContext={!!CANONICAL_CONTEXT} readOnly />
    <ContextBar projection={CURRENT} selected={selected} readOnly />
    <div className={mergeClasses(s.detail, s.sourceDetail, s.overviewDocumentContent)}>
      <WorkspaceViewPanels view="overview" viewId={viewId} hasContext={!!CANONICAL_CONTEXT}>
    ...
    <div className={mergeClasses(s.page, s.app)}>
      ...
      <div className={mergeClasses(s.viewport, settings.width !== 'fluid' && s[`width${settings.width}`])}>
        <section className={mergeClasses(s.product, reviewOpen && s.hidden)} aria-label="Needs you workspace">
          ...
          {currentWorkspaceView(view) !== 'needs' && <ContextBar projection={projection} selected={contextId}
            onCommit={commitContext} onBrowse={() => setBrowseOpen(true)} />}
            ...
            <div className={s.stack}>
              ...
              {view === 'new' ? <NewIdea value={value} onChange={updateDraft} onCancel={openNeeds} />
                : view === 'overview' ? <Overview snapshotKey={settings.snapshot} projection={projection}
                  onOpen={openOverviewContext} onNew={newIdea} />
                : request ? <RequestForm key={`${request.id}-${settings.permission}`} request={request} value={value} onChange={updateDraft}
                  ...
                  : view === 'context' ? inventoryUnavailable(projection) ? <Notice intent="error" title="Workspace inventory unavailable">
                    ...
                    reviewCondition={settings.review} />
