...
- [ ] Exercise exact lower and upper bounds for findings, alternatives, failed approaches, evidence, assumptions, subjects, events, batches, governance bytes, source bytes, Inspection bytes, packet items, and CLI bytes; prove completion adds no larger limit, paging, accumulator, API, or state field.
...
- [ ] Gate every fresh Inspection required by `complete.capture` or `complete.finalize` before trusted normalization, batch validation, retention classification, or successor-state construction.
- [ ] At both packet boundaries, require the existing descriptor-only `overflow:true`, `evidence-incomplete` / `model-packet` refusal with no model packet.
- [ ] Prove capture overflow preserves exact pre-call RunState and authoritative surfaces and creates no `pendingCompletion`, batch, completion, occurrence count, repeat, governance, no-progress, or retry authority.
- [ ] Prove finalize overflow preserves exact pre-call RunState, `pendingCompletion`, pending attempt, and already-retained occurrence records and admits or derives no new authority.
...
- [ ] Return `learning-governance-capacity` on the seventeenth basis only when failed-set excess is derivable from fully admitted evidence or already-authoritative bounded state; keep descriptor-only overflow `evidence-incomplete` / `model-packet` and authorize neither no-progress nor retry.
...
- [ ] Refuse overwrite by another target with the applicable existing retention or governance conflict; never use `learning-governance-capacity` for singleton occupancy.
...
- [ ] Exercise descriptor-only packet overflow through both serialized completion modes and prove exact state and authoritative-surface preservation.
