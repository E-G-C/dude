...
- [ ] Enforce source, aggregate, packet, event, event-line, batch, governance, collection, and request limits before expensive work; add no larger limit, paging, accumulator, completion API, or state field to bypass them.
...
- [ ] Treat every fresh Inspection acquired by `complete.capture` or `complete.finalize` as an earlier fail-closed gate for that call.
- [ ] Return packet overflow through the existing descriptor-only `evidence-incomplete` / `model-packet` refusal with no model packet and exact pre-call authoritative surfaces.
- [ ] Prevent capture overflow from creating or replacing `pendingCompletion`, a projection batch, completion admission, occurrence count, repeat, governance, no-progress, or retry authority.
- [ ] On finalize overflow, preserve exact `pendingCompletion`, pending attempt, and already-retained occurrence records; admit or count no completion and derive no repeat, governance, no-progress, or new-attempt authority.
...
- [ ] Return `learning-governance-capacity` only for failed-set excess derivable from fully admitted evidence or already-authoritative bounded state; never relabel descriptor-only packet overflow, and authorize neither no-progress nor retry.
