...
5. Repeat while crossing the existing Inspection packet item and byte boundaries.
...
- the overflow case gates before trusted normalization or event, batch, and successor-state derivation and returns the existing descriptor-only `evidence-incomplete` / `model-packet` refusal;
- capture overflow preserves exact pre-call RunState and authoritative surfaces and creates no `pendingCompletion`, batch, completion, repeat, governance, no-progress, or retry authority; and
- no larger limit, paging, accumulator, completion API, or state field appears.
...
7. With the events already retained and completion still pending, repeat finalize while its fresh Inspection crosses the existing packet boundary.
...
- finalize overflow returns the existing descriptor-only `evidence-incomplete` / `model-packet` refusal before batch or retention classification;
- exact pre-call RunState, `pendingCompletion`, pending attempt, and retained occurrence records remain unchanged; and
- no completion is admitted or counted and no repeat, governance, no-progress, or new-attempt authority is derived.
...
- the seventeenth basis returns `learning-governance-capacity` only when failed-set excess is derivable from fully admitted evidence or already-authoritative bounded state;
- descriptor-only packet overflow remains `evidence-incomplete` / `model-packet` and is never relabeled as failed-set capacity; and
- neither outcome authorizes no-progress or retry.
...
- derivable failed-set capacity exhaustion remains unresolved rather than becoming no-progress or authorizing retry.
