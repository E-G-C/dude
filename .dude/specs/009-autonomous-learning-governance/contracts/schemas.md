...
- Existing source, Inspection, packet, aggregate-byte, and CLI request limits remain authoritative and are not enlarged; completion adds no paging, accumulator, API, or state field to bypass them.
...
- every fresh Inspection acquired by `complete.capture` or `complete.finalize` is an earlier call-local gate before trusted normalization, batch validation, retention classification, or successor-state construction;
- packet overflow returns the existing descriptor-only Inspection refusal: `overflow:true`, no item text, blocker `code:"evidence-incomplete"`, `subject:"model-packet"`, matching `evidenceHash`, and no model packet;
- capture overflow preserves exact pre-call RunState and authoritative surfaces and does not create or replace `pendingCompletion`, a batch, completion admission, an occurrence count, a repeat, governance, no-progress, or retry authority; and
- finalize overflow preserves exact pre-call RunState and authoritative surfaces, including `pendingCompletion`, the pending attempt, and already-retained occurrence records, while admitting or counting no completion and deriving no repeat, governance, no-progress, or new-attempt authority.

After an admitted unblocked Inspection, `complete.capture` normalizes the trusted envelopes, validates all bindings, derives one approach event plus the complete finding-event set, returns the exact ordered batch, stores only its commitment, and leaves the attempt pending and uncounted.

After an admitted unblocked Inspection, `complete.finalize` validates the caller-supplied exact batch against `pendingCompletion`, requires each event exactly once and byte-equivalent on both surfaces, and only then admits the completion. It derives the earliest valid repeat after admission. An immediate halt before retention leaves completion pending and establishes no repeat. An immediate halt after retention but before governance projection leaves exact evidence for deterministic re-derivation.
...
After closed request-shape validation, fresh Inspection acquisition is the first semantic gate and precedes semantic use of `completion`. Packet overflow returns through the existing bounded descriptor-only Inspection refusal with exact request RunState and authoritative surfaces; the Success shape below is not produced.
...
After closed request-shape validation, fresh Inspection acquisition is the first semantic gate and precedes semantic use of `projectionBatch` or retained occurrence evidence. Packet overflow returns through the same existing descriptor-only refusal and preserves exact request state, `pendingCompletion`, the pending attempt, and retained records.
...
A non-overflow finalization failure leaves `captured:true`, `finalized:false`, the attempt pending, and state unresolved. Inspection overflow returns earlier and produces no completion-specific successor result or authority.
...
| A fresh `complete.capture` or `complete.finalize` Inspection packet overflows | Return the existing descriptor-only `evidence-incomplete` / `model-packet` refusal; preserve exact pre-call RunState and authoritative surfaces and derive no completion, repeat, governance, no-progress, or attempt authority. |
| Failed-set excess is visible only beyond a descriptor-only packet overflow | Keep the overflow refusal; do not return `learning-governance-capacity`. |
| Failed-set excess is derivable from fully admitted evidence or already-authoritative bounded state | Return `learning-governance-capacity`; authorize neither no-progress nor retry. |
