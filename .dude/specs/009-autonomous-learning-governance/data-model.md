...
- Completion retains the existing Inspection packet limit and descriptor-only overflow model; it adds no larger limit, paging, accumulator, completion API, or RunState field.
...
stateDiagram-v2
  ...
  pending_attempt --> pending_attempt: capture Inspection overflow / exact pre-call state
    pending_attempt --> retention_pending: complete.capture
    retention_pending --> retention_pending: partial or failed projection
  retention_pending --> retention_pending: finalize Inspection overflow / exact pre-call state
...
Each call validates its newly acquired Inspection as an earlier fail-closed gate before trusted normalization, batch validation, retention classification, or successor-state construction. Capture packet overflow returns the existing descriptor-only `evidence-incomplete` / `model-packet` refusal with no model packet, preserves exact pre-call RunState and authoritative surfaces, and creates no `pendingCompletion`, batch, completion admission, occurrence count, repeat, governance, no-progress, or retry authority.

On an admitted Inspection, capture derives exact occurrence events but does not clear the pending attempt, add a completed attempt, count an occurrence, derive repetition, create governance, or authorize lane disposition. Finalize packet overflow preserves exact pre-call RunState, `pendingCompletion`, the pending attempt, and already-retained occurrence records on both surfaces; it admits or counts no completion and derives no repeat, governance, no-progress, or new-attempt authority. On an admitted Inspection, finalize admits completion only after every committed occurrence is present exactly once and byte-equivalent on both surfaces. An immediate halt before retention leaves completion pending. A halt after retention may leave required governance re-derivable.
...
For finding triggers, members come from every qualifying finding event's `attemptApproachBasisIdentity`. For approach triggers, members come from the qualifying approach events. Later rejected selected alternatives extend the set; no member is removed. `learning-governance-capacity` applies only when failed-set excess is derivable from fully admitted evidence or already-authoritative bounded state; descriptor-only packet overflow remains the existing `evidence-incomplete` / `model-packet` refusal. Capacity authorizes neither no-progress nor retry.
