...
- Existing source, Inspection, packet, event, and CLI limits remain authoritative; completion does not enlarge them or add paging, an accumulator, another API, or another state field
...
3. Acquire and gate each required fresh Inspection, validate complete source authority, and apply existing blockers before call-specific semantic work.
...
A second target requiring an occupied singleton slot returns the applicable existing `occurrence-retention-conflict` or `learning-governance-conflict` without overwriting evidence; it is not `learning-governance-capacity`.
...
2. Apply its existing fail-closed gate before trusted-source normalization or any state, event, or batch derivation.
3. If the packet overflows, return the existing descriptor-only packet-overflow refusal with `overflow:true`, blocker `code:"evidence-incomplete"` and `subject:"model-packet"`, and no model packet. Preserve exact pre-call RunState and authoritative surfaces; create no `pendingCompletion`, batch, completion admission, occurrence count, repeat, governance, no-progress, or retry authority.
4. Resolve the caller's identity references to trusted verification and independent-review captures.
5. Normalize both envelopes and recompute every identity.
6. Validate target, attempt, result, source revision, check definition, verdict, and chronology bindings.
7. Derive one immutable `ApproachOccurrenceEventV1`.
8. Derive every immutable `FindingOccurrenceEventV1` from the complete independent-review envelope.
9. Return a bounded exact `ProjectionBatchV1` with purpose `occurrence-retention`.
10. Record only the batch commitment in `pendingCompletion`.
11. Do not clear the pending attempt, add a completed row, count an occurrence, or derive repetition.

The Work host appends current-run records and the lane owner appends canonical lane records for every event.

#### `complete.finalize`

1. Reacquire one fresh Inspection containing both authoritative surfaces.
2. Apply its existing fail-closed gate before supplied-batch validation, retention classification, or successor-state derivation.
3. If the packet overflows, return the same existing descriptor-only `evidence-incomplete` / `model-packet` refusal. Preserve exact pre-call RunState and surfaces, including `pendingCompletion`, the pending attempt, and already-retained occurrence records; admit or count no completion and derive no repeat, governance, no-progress, or new-attempt authority.
4. Require every committed occurrence event exactly once and byte-equivalent on both surfaces.
5. Revalidate the supplied exact batch against `pendingCompletion`.
6. Only then clear the pending attempt and add the completed attempt.
7. Count retained occurrence events and derive the earliest valid Repeat Relationship.
8. If no repeat exists, return the ordinary normalized completion result.
9. If a repeat exists, create `learningGovernance` in `required`, derive the complete failed-approach set, and return `learning-required` plus the exact governance-required projection batch.
...
Every later rejected selected-alternative attempt extends the set. No member is discarded. A failed-set excess returns `learning-governance-capacity` only when the excess is derivable from fully admitted evidence or already-authoritative bounded state. Descriptor-only packet overflow remains `evidence-incomplete` / `model-packet`, and evidence conflict remains conflict; neither is relabeled. `learning-governance-capacity` authorizes neither no-progress nor retry.
...
| `complete.capture` | Gate its fresh Inspection first, then normalize trusted completion evidence and return immutable occurrence events without counting them. |
| `complete.finalize` | Gate its fresh Inspection first, then verify dual occurrence retention, count completion, and derive repetition. |
