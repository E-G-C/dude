...
- In the dogfood repo, live compose operations are permitted only for this
  explicit set: `a2a`, `authoring`, `coding`, `design`, `release`,
  `rubber-duck`, `strata`, and `writing`. Test any other catalog pack in a
  throwaway root (`--root <tmp>`).
