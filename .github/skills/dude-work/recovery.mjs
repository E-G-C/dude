...
// Closed per-phase rules for every reachable phase. Optional suspension data
// remains valid for carried RunState, while controlled end stays limited to
// alternative-inspected and no-progress-verified.
...
// === Feature 009 (T004): retained governance validation and versioned audit ===
// Existing suspension and terminal outcome records remain valid, and ordinary
// audit derives evidence only from the byte-equivalent dual-surface intersection.
 ...
 * @param {unknown} [dependencies] @param {boolean} [transport]
 */
export function auditGovernanceV2(stateValue, inputValue, dependencies, transport = false) {
  const state = /** @type {Record<string, unknown>} */ (validateRunState(stateValue));
  if (/** @type {Record<string, unknown>} */ (state.policy).mode !== 'autonomous') {
  ...
  const suspension = Object.hasOwn(governance, 'suspension')
    ...
    : null;
  const invocationOutcome = controlledEnd ? 'controlled-unresolved-end' : 'in-progress';
  /** @type {Record<string, unknown>|null} */
  ...
  const summaryWithoutIdentity = {
    ...
    invocationOutcome,
    scope: 'target',
    unresolvedReason: controlledEnd ? 'controlled-unresolved-end' : suspension ? suspension.reason : 'learning-required',
    schedulingOutcome: suspension ? 'sequential-disjoint-continuation' : 'none',
...
export function runCommand(commandValue, requestValue, dependencies) {
  ...
  if (command === 'audit') {
    const request = assertExactRecord(requestValue, ['state', 'input'], [], 'audit request');
    const input = decodeTransportInput(request.input, 'audit request.input', false);
    return auditGovernanceV2(request.state, input, dependencies, true);
  }
  if (command === 'inspect') {
  ...
  if (command === 'transition') {
    ...
    assertEnum(
      ...
      [
        ...
        'controlled-end', 'resume-governance',
...
export function deriveGovernanceRuntimeRequestV1(stateValue, intentValue) {
  ...
  const governed = assertExactRecord(intent, ['action', 'input'], [], 'GovernanceRuntimeRequestV1.intent');
  const routes = {
...
// === Retained definition-recovery descriptors and RunState validation ======
// Write-set and file-state identities remain available to definition recovery.
// Existing evaluation-sequence and learning-review references remain bounded
// and validated even though production Work creates no evaluation sequence.
...
// --- Bounded learning events and shared dual projection ---------------------
// Learning-review events remain bounded canonical records. Projection writes
// and fresh reads continue through the injected owner without a second ledger.
...
/** Validate one projected learning event. @param {unknown} value @param {string} [label] */
function validateEvent(value, label = 'event') {
  ...
  return invalid(label + '.type', 'must be learning-review');
}
...
// --- Retained RunState rows and learning-review references ------------------
