...
- Release bundles are produced by `scripts/build-release.mjs` into `dist/` and ship the core tier only: they exclude `library/` (packs) and all `*.test.mjs`, and emit a generic `project` skill stub plus a seeded `bundle-manifest.md` (installed_sha/installed_at rewritten; `source_ref` left as `main`). Only `bundle-manifest.md` and `profile.md` ship under `.github/dudestuff/`; dev memory (decisions/context/lessons) is not shipped.
